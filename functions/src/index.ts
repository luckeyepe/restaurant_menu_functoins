    ...
    .onUpdate(async (change, context) => {
        ...
        if (itemAfter.cart_item_count === itemBefore.cart_item_count){
            ...
            if (change.after !== undefined && change.before !== undefined) {
                const brokenPath: string[] = change.after.ref.path.split("/");
                const invokerUid: string = brokenPath[1];

                const cartData = itemAfter;
                // @ts-ignore
                const cartCost = cartData.cart_item_price * cartData.cart_item_count;
                ...
                if (itemAfter.cart_item_count < itemBefore.cart_item_count) {
                    ...
                    currentItemCount = currentItemCount - countBefore + countAfter;
                    ...
                    // @ts-ignore
                    const countBefore = itemBefore.cart_item_count;
                    currentItemCount = currentItemCount - countBefore + count;
                    // @ts-ignore
                    const cartCostBefore = itemBefore.cart_item_price * itemBefore.cart_item_count;
...
export const updateClartDetailsOnCartDelete = functions.firestore
    .document("Cart/{userUid}/cart/{cartUid}")
    .onDelete(async (snapshot, context) => {
        ...
        // @ts-ignore
        const cartCount = cartData.cart_item_count;

        return admin.firestore()
            .doc("Cart_Details/"+invokerUid)
            .update({
                cart_detail_cost: currentCost-cartCost,
                cart_detail_item_count: currentItemCount- cartCount
            })
    });